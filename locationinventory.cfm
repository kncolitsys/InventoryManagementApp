...
<cfclient>
  	<cfscript>  		
		...
		public any function latestInventoryLoad(){
			if (utils.getQueryVariable('l')) {				
				var loc = decodeURIComponent(utils.getQueryVariable('l'));
				inventoryNS.getInventoryByLocation(loc);		 
			} else {
				window.history.go(-1);
			}
		} 
  ...
  <body>
	...
	<div class="container" style="padding-top: 25px;">
		...
		<hr />
			<div>
				<div class="col-xs-12">
					<button type="button" class="btn btn-success btn-lg btn-block" onclick="history.go(-1);"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;&nbsp;&nbsp;Back to Locations</button>
				</div>
			</div>
      <!-- /container -->
    </div>
