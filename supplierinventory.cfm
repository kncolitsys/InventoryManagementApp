...
<cfclient>
  	<cfscript>  		
		...
		public any function inventoryLoad(){
			if (utils.getQueryVariable('s')) {				
				var loc = decodeURIComponent(utils.getQueryVariable('s'));
				inventoryNS.getInventoryBySupplier(loc);								 
			} else {
				window.history.go(-1);
			}
		} 
   ...
   <head>
    <title>CFInventory - Inventory</title>
	    ...
	    <script id="inventory-list-template" type="text/x-handlebars-template">	    
	    	<div class="alert alert-info" style="text-align: center;"><strong>Inventory for supplier: <br>{{records.0.name}}</strong></div><hr>
	    	{{#each records}}
	    	<div class="media">
	    ...
	    <script id="inventory-search-noResults" type="text/x-handlebars-template">
	    	<div class="alert alert-warning" style="text-align: center;"><strong>The supplier has no inventory.</strong></div>
	    	<hr>
	    </script>
  ...
  <body>
    ...
    <div class="container" style="padding-top: 25px;">
		...
		<hr />
			<div>
				<div class="col-xs-12">
					<button type="button" class="btn btn-success btn-lg btn-block" onclick="history.go(-1);"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;&nbsp;&nbsp;Back to Suppliers</button>
				</div>
			</div>
      <!-- /container -->
    </div>
